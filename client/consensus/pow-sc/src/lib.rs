...
//pub type Difficulty = TotalDifficulty + Default + Encode + Decode + Ord + Clone + Copy;
...
pub struct PowBlockImport<B: BlockT, I, C, D, S> {
	inner: I,
	select_chain: Option<S>,
	client: Arc<C>,
	inherent_data_providers: sp_inherents::InherentDataProviders,
	check_inherents_after: <<B as BlockT>::Header as HeaderT>::Number,
	_difficuty: PhantomData<D>
}

impl<B: BlockT, I: Clone, C, D: Clone, S: Clone> Clone for PowBlockImport<B, I, C, D, S> {
	fn clone(&self) -> Self {
		Self {
...
impl<B, I, C, D, S> PowBlockImport<B, I, C, D, S> where
	B: BlockT,
	...
	D: TotalDifficulty + Default + Encode + Decode + Ord + Clone + Copy,
{
	...
	pub fn new(
		...
		Self { inner, client, check_inherents_after,
			   select_chain, inherent_data_providers, _difficuty: PhantomData::<D> }
	}
...
impl<B, I, C, D, S> BlockImport<B> for PowBlockImport<B, I, C, D, S> where
	B: BlockT,
	I: BlockImport<B, Transaction = sp_api::TransactionFor<C, B>> + Send + Sync,
	I::Error: Into<ConsensusError>,
	S: SelectChain<B>,
	...
	D: TotalDifficulty + Default + Encode + Decode + Ord + Clone + Copy,
{
	...
	fn import_block(
		...
		let intermediate = block.take_intermediate::<PowIntermediate::<D>>(
			INTERMEDIATE_KEY
		)?;
...
pub struct PowVerifier<B: BlockT, D> {
	_marker: PhantomData<B>,
	_difficulty: PhantomData<D>,
}

impl<B: BlockT, D> PowVerifier<B, D> {
	pub fn new(
	) -> Self {
		Self { _marker: PhantomData, _difficulty: PhantomData }
	}
	...
	D: TotalDifficulty + Default + Encode + Decode + Ord + Clone + Copy,
{
	fn verify(
		...
		let intermediate = PowIntermediate::<D> {
			difficulty: None,
		};
...
pub fn import_queue<B, Transaction>(
...
pub fn start_mine<B: BlockT, C, E, SO, S, CAW>(
...
fn mine_loop<B: BlockT, C, D, E, SO, S, CAW>(
	...
	CAW: CanAuthorWith<B>,
{
	'outer: loop {
		...
		let (difficulty, seal) = {
			...
			let difficulty = Default::default();
			loop {
				let seal = Some(vec![]);
