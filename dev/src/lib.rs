...
pub use sc_network::config::{NodeKeyConfig, Secret, Ed25519Secret};
use sp_core::H256;
use std::str::FromStr;
...
pub fn get_bootnodes_router_params(shard_count: Option<u16>)-> error::Result<Vec<BootnodesRouterParams>>{
    ...
    Ok(shard_conf_map.iter().map(|(k, v)| {
        ...
        let port = (*v).3;
        let node_key = v.4;
        let node_key_config = NodeKeyConfig::Ed25519(parse_ed25519_secret(&node_key.to_string()).unwrap());
...
pub fn parse_ed25519_secret(hex: &String) -> error::Result<Ed25519Secret> {
    H256::from_str(hex).map_err(invalid_node_key).and_then(|bytes|
        ed25519::SecretKey::from_bytes(bytes).map(Secret::Input).map_err(invalid_node_key)
    )
}
